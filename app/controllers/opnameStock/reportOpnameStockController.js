...
import { reportOpnameStockWorkbook } from '../../helpers/xls/reportOpnameStockWorkbook'
...
function getOpnameStockCondition(params) {
  let {
    year, start_date, end_date, entity_id,
  } = params
...
function formatListEntityOpname({ entities, opnameStocks, months }) {
  // opnameStocks
  const overviews = {}
  ...
  entities.forEach((entity) => {
    ...
    // parse opnamestocks to overview
    let total = 0
    opnameStocks.filter((el) => el.entity_id === entity.id)
      .forEach((el) => {
        total += el.value
...
function formatBarData({ opnameStockByMonth, year, months }) {
  const intervalPeriod = []
  const column = []
  const overview = []
  months.forEach((i) => {
    const month = leadZeroNumber(i)
    const data = opnameStockByMonth.find((el) => moment(el.created_at).format('MM') === month)
...
async function queryReportOpnameStockByEntity(options) {
  const reportByEntity = await models.OpnameStock.findAll({
    attributes: [
      'entity_id',
      'created_at',
      [sequelize.fn('Month', sequelize.literal('`OpnameStock`.`created_at`')), 'month'],
      [sequelize.fn('Year', sequelize.literal('`OpnameStock`.`created_at`')), 'year'],
...
async function queryReportOpnameStockByMonth(options) {
  const reportByMonth = await models.OpnameStock.findAll({
    attributes: [
      'created_at',
      [sequelize.fn('Month', sequelize.literal('`OpnameStock`.`created_at`')), 'month'],
      [sequelize.fn('Year', sequelize.literal('`OpnameStock`.`created_at`')), 'year'],
...
export async function entityReport(req, res, next) {
  try {
    ...
    const { condition: opnameStockCondition, months } = getOpnameStockCondition(req.query)
    const opnameStocks = await queryReportOpnameStockByEntity({ where: opnameStockCondition })

    const { list, intervalPeriod } = formatListEntityOpname({ entities, opnameStocks, months })
    const data = {
      month: '',
      intervalPeriod,
      total,
...
export async function barReport(req, res, next) {
  try {
    const { condition, year, months } = getOpnameStockCondition(req.query)

    const entityOptions = getEntityOptions(req.query)

    const opnameStockByMonth = await queryReportOpnameStockByMonth({
      where: condition,
      include: [{
        association: 'entity',
        ...entityOptions,
    ...
    const data = formatBarData({ opnameStockByMonth, year, months })
...
export async function entityReportXLS(req, res, next) {
  try {
    const { province_id, regency_id, entity_id } = req.query
    let province = null
    let regency = null
    let entity = null
    ...
    const { condition: opnameStockCondition, months, year } = getOpnameStockCondition(req.query)
    const opnameStocks = await queryReportOpnameStockByEntity({
      where: opnameStockCondition,
      include: [{
        association: 'entity',
        ...entityOptions,
      }],
    ...
    const formatList = formatListEntityOpname({ entities, opnameStocks, months })

    const workbook = await reportOpnameStockWorkbook({
      ...formatList,
      year,
      province,
      regency,
    ...
    const filename = `Laporan Aktivitas Reconciliation (${timestamp})`
