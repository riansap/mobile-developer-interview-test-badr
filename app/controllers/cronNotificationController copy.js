...
export async function createNotifRabiesVaccine(entityID) {
  ...
  let conditions = {}
  if (entityID) conditions = { where: { entity_id: entityID } }
  ...
  for (let rule of rules) {
    ...
    if (sequence < 6)
      ...
      conditions2.push({ days2: { [Op.between]: [start_notification, end_notification] }, preexposure_sequence: sequence })
      ruleExp = rule
    }
  }
  ...
  for (let patient of Patients) {
    ...
    let is_preexposure = (days2 >= ruleExp.start_notification && days2 <= ruleExp.end_notification) && patient?.preexposure_sequence == 6
